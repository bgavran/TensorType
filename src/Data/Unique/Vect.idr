...
%hide Misc.NotElem.NotElem

-- `UniqueVect` and `NotElem` are defined in terms of each other
mutual
  ...
  public export
  data NotElem : DecEq a => (x : a) -> (xs : UniqueVect n a) -> Type where
    NotInEmptyVect : DecEq a => (x : a) -> NotElem {a=a} x []
    NotInNonEmptyVect : (de : DecEq a) =>
      {x, y : a} ->
      (xs : UniqueVect n a) ->
      (ne : NotElem x xs) ->
      (neq : IsNo (decEq x y)) =>
