...
namespace UniqueList
  mutual
    ...
    public export
    data UniqueList : (0 a : Type) -> DecEq a => Type where
      Nil : {0 a : Type} -> DecEq a => UniqueList a
      (::) : {0 a : Type} -> DecEq a =>
        (x : a) ->
        (xs : UniqueList a) ->
    ...
    data NotElem : DecEq a =>
      ...
      NotInEmptyList : {0 a : Type} -> DecEq a => (x : a)
        -> NotElem {a=a} x []
      NotInNonEmptyList : {0 a : Type} -> (de : DecEq a) =>
        {x, y : a} ->
        (xs : UniqueList a) ->
        NotElem x xs ->
  ...
  public export
  decElemNotInUniqueList : {0 a : Type} -> DecEq a =>
  ...
  public export
  toList : DecEq a => UniqueList a -> List a
  toList [] = []
  toList (x :: xs) = x :: toList xs
  
  ||| Convert a list to an unique list, removing duplicates
  public export
  fromList : DecEq a => List a -> UniqueList a
  fromList [] = []
  ...
  public export
  length : DecEq a => UniqueList a -> Nat
  length = length . toList
 
  ||| Convert a list to an unique list, throw an error if there are duplicates
  public export
  fromListMaybe : DecEq a => List a -> Maybe (UniqueList a)
  fromListMaybe [] = Just []
  ...
  public export
  fromVect : DecEq a => Vect n a -> UniqueList a
  fromVect [] = []
  ...
  public export
  toVect : DecEq a => UniqueList a -> (n : Nat ** Vect n a)
  toVect [] = (0 ** [])
